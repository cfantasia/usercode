...
//-----------------------------------------------------------
 ...
 FitAllfunc->SetParameter(2,10); FitAllfunc->SetParName(3,"Half Width");
 FitAllfunc->SetParameter(3,91); FitAllfunc->SetParName(4,"Z peak");
 FitAllfunc->SetParameter(4,10); FitAllfunc->SetParName(5,"Breit Norm");
...
//----------------------------------------------------------
    ...
    /*
      Cory: When data is available.
      1) Fit background outside Z peak to linear func
      2) Fit zpeak with BW peak + linear
      3) How many events in Z peak according to linear
    */    
    ...
    TH1F* allhist = get_sum_of_hists(allfiles,true,"_AllCuts");//_mw

    //2.shape the background function
    //Cory: NEED GLOBAL IGNORE POINTS!!!!
    TF1* nozpeakfunc;
    ...
    TH1F* zpeakhist = get_sum_of_hists(allfiles,true,"_AllCuts");//_Zcut  
   ...
   // write the fit results into the par array
    Double_t allfunc_pars[5];
    Double_t allfunc_errs[5];
